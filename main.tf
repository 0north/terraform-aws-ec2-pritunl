    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${trimprefix(local.ssm_path_default_credentials, "/")}",
          "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${trimprefix(local.ssm_path_key_pair, "/")}"
        ]
      },
      {
        Effect = "Deny",
